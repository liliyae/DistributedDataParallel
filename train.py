...
parser.add_argument('--bsize', dest='bsize', default=256, type=int)
...
parser.add_argument('--maxsteps', dest='maxsteps', default=500, type=int)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = ABERT(args.config, dev=args.gpu, doc_maxlen=args.doc_maxlen,
                  dim=args.dim, mask_punctuation=args.mask_punctuation, similarity_metric=args.similarity)
    ...
    if args.distributed:
        ...
        if args.gpu is not None:
            ...
            args.bsize = int(args.bsize / ngpus_per_node)
    ...
    train_dataset = pretrain_DataLoader(config=args.config, doc_maxlen=args.doc_maxlen)
    ...
    train_loader = torch.utils.data.DataLoader(
        train_dataset, batch_size=args.bsize, shuffle=(train_sampler is None),
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, data in enumerate(train_loader):
        ...
        maskmatrix = data['maskmatrix'].cuda()
        mat = data['mat'].cuda()

        # compute output
        target, output = model(input_ids, attention_mask, maskmatrix, mat)
        ...
        if args.local_rank == 0:
            logger.info('batch {1}\ttrain/avg_loss = {0}'.format(loss.item(), i))
    ...
    if args.local_rank == 0:
        # 只在Process0中保存模型
        output_model_file = "/home/sdu/dyl/pretrain/ABERT/mybert/pytorch_model.bin"
